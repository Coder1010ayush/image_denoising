...
class UDnCNN(NNRegressor):
    ...
    def forward(self, x):
        ...
        for i in range(D // 2 - 1):
            ...
            h, idx = F.max_pool2d(
                F.relu(self.bn[i](self.conv[i + 1](h))),
                kernel_size=(2, 2),
                return_indices=True,
            )
            h_buff.append(h)
        ...
        for i in range(D // 2 + 1, D):
            j = i - (D // 2 + 1) + 1
            h = F.max_unpool2d(
                F.relu(self.bn[i](self.conv[i + 1]((h + h_buff[-j]) / np.sqrt(2)))),
                idx_buff[-j],
                kernel_size=(2, 2),
                output_size=shape_buff[-j],
            )
        y = self.conv[D + 1](h) + x
        return y
...
class DUDnCNN(NNRegressor):
    ...
    def __init__(self, D, C=64):
        ...
        self.conv.extend(
            [
                nn.Conv2d(C, C, 3, padding=dilations[i + 1], dilation=dilations[i + 1])
                for i in range(D)
            ]
        )
        self.conv.append(
